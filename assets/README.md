...
| `portrait.svg` | Monochrome ASCII portrait that types itself in (SMIL). Generated from `portrait-src.png`. |
| `portrait-src.png` | Background-removed cutout used by `portrait.py` (made with `rembg`). |
| `portrait.py` | `python assets/portrait.py` → regenerates `portrait.svg` + `_portrait_preview.png`. Adjust `CROPF` / `COLS` at the top. For a new photo: `rembg i new.jpg assets/portrait-src.png` first. |
